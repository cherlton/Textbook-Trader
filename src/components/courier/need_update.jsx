...
const CourierDashboard = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setProfile(response.data.courier);
      } catch (error) {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      setLoading(true);
      try {
        let url = `${BACKEND_URL}/api/courier/orders`;
        if (filter !== 'all') {
          url += `?status=${filter}`;
        }
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      await axios.put(
        `${BACKEND_URL}/api/courier/orders/${orderId}/status`,
        { status: newStatus },
    ...
    <div className="bg-white rounded-lg shadow-md p-4 mb-4 hover:shadow-lg transition-shadow">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-gray-600">{order.books}</p>
      ...
      <div className="mt-3 pt-3 border-t border-gray-100 flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-600">
            <span className="font-medium">Contact:</span> {order.recipient_contact}
          </p>
        </div>
  ...
  const OrderDetailsModal = ({ order, onClose }) => {
    ...
    useEffect(() => {
      const fetchOrderDetails = async () => {
        try {
          const response = await axios.get(`${BACKEND_URL}/api/courier/orders/${order.id}`, { withCredentials: true });
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <p className="text-gray-600">{details.first_name} {details.last_name}</p>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <p className="text-gray-600">
                      <span className="font-medium">Buyer:</span> {details.buyer_name} ({details.buyer_contact})
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-4">
                  ...
                  <div className="space-y-3">
                    {details.items.map((item) => (
                      <div key={item.book_id} className="flex items-start p-3 border-b border-gray-100">
                        {item.image_path && (
                          <img
  ...
  const ProfileSection = () => {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        setProfile(response.data.updated_fields);
